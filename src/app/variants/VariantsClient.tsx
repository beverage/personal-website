...
const backgroundVariants: {
	...
	},
	{
		...
		description:
			'50 center stars with 2x brightness + 1.4x size at same distance',
	},
	{
		name: 'Close Center 1',
	...
	},
	{
		...
		description:
			'80 normal center stars at 2.5-6x closer distance + more concentrated',
	},
]
...
const uiVariants: { key: UiVariant; name: string; description: string }[] = [
	{
		key: 'hero-middle',
		name: 'Hero Middle Button',
		description: 'Add CV as the middle button in the hero row (outlined style)',
	},
	{
		key: 'footer-icon',
		name: 'Footer Icon',
		description: 'Add CV icon link in footer (opens in new tab)',
	},
	{
		key: 'float-pill',
		name: 'Floating Pill',
		description: 'Top-right floating “CV” pill with icon',
	},
]

export function VariantsClient({ cvUrl, clientConfig }: VariantsClientProps) {
	const [selectedBackground, setSelectedBackground] = useState<ClusterVariant>(
	...
	const backgroundDescription = useMemo(
		() =>
			backgroundVariants.find(v => v.variant === selectedBackground)
				?.description,
		[selectedBackground],
	)
		...
		>
			...
			>
				...
				<div style={{ marginBottom: 15 }}>
					...
					<select
						...
						onChange={e =>
							setSelectedBackground(e.target.value as ClusterVariant)
						}
						style={{
							width: '100%',
							padding: 8,
				...
				<div style={{ marginBottom: 15 }}>
					...
					<select
						value={selectedUi}
						onChange={e => setSelectedUi(e.target.value as UiVariant)}
						style={{
							width: '100%',
							padding: 8,
							background: '#222',
				...
				<div style={{ marginBottom: 15 }}>
					<label style={{ display: 'block', marginBottom: 5, fontSize: 14 }}>
						Cluster Opacity: {opacity.toFixed(1)}
					</label>
					<input
						type="range"
						min="0"
			...
			>
				...
				>
					{backgroundVariants.map(v => (
						<button
							key={v.variant}
							onClick={() => setSelectedBackground(v.variant)}
							style={{
								padding: '8px 12px',
								background:
									selectedBackground === v.variant ? '#111827' : '#222',
								border:
									selectedBackground === v.variant
				...
				>
					{uiVariants.map(u => (
						<button
							key={u.key}
							onClick={() => setSelectedUi(u.key)}
							style={{
								padding: '8px 12px',
								background: selectedUi === u.key ? '#111827' : '#222',
								border:
									selectedUi === u.key
										? '1px solid rgba(34,211,238,0.85)'
										: '1px solid #333',
				...
				<>
					...
					{selectedUi !== 'hero-middle' && (
						<div className="relative z-10 flex min-h-screen items-center justify-center px-6">
							<HeroSection
								title="Alex Beverage"
								description="Software engineer building LLM products"
							/>
						</div>
					)}
			...
			{uiVisible && (
				<>
							...
							<div className="max-w-5xl text-center">
								<div className="mb-8 p-12">
									...
									<p className="font-exo2 mx-auto mb-8 max-w-2xl text-xl leading-relaxed text-white/90 sm:text-2xl">
										Software engineer building LLM products
									</p>
					...
					{selectedUi === 'footer-icon' && (
						<div className="absolute bottom-8 left-1/2 z-50 -translate-x-1/2">
							<div className="rounded-full border border-white/10 bg-black/20 px-6 py-3 backdrop-blur-sm">
								<div className="flex items-center space-x-6">
									<div className="text-sm text-white/60">
										© {clientConfig?.copyrightYear ?? new Date().getFullYear()}
									</div>

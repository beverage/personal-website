	...
	config.gradientStops.forEach((stop, index) => {
		...
		if (index === 0) {
			gradient.addColorStop(
				stop,
				`rgba(255, ${255 * colorShift}, ${255 * colorShift}, ${opacity * twinkle * opacityMultiplier})`,
			)
		} else if (index === 1) {
			gradient.addColorStop(
				stop,
				`rgba(${255 * colorShift}, ${255 * colorShift}, 255, ${opacity * twinkle * opacityMultiplier})`,
			)
		} else if (index === 2) {
			gradient.addColorStop(
				stop,
				`rgba(${200 * colorShift}, ${220 * colorShift}, 255, ${opacity * twinkle * opacityMultiplier})`,
			)
		} else {
...
export const getClusterVariantInfo = (variant: ClusterVariant) => {
	...
	const info = {
		'cluster-ellipse-4x': {
			...
			description:
				'Horizontal ellipse with 4:1 ratio (a=4r, b=r) - Lenticular star cluster',
			isWinner: true,
		},
		'cluster-ellipse-4x-center-bright-1': {
			title: 'Elliptical 4× + Bright Center 1',
			description:
				'Lenticular cluster with 60 bright center stars (2.5x intensity)',
			isWinner: false,
		},
		'cluster-ellipse-4x-center-bright-2': {
			title: 'Elliptical 4× + Bright Center 2',
			description:
				'Lenticular cluster with 50 bright + larger center stars (2x intensity, 1.4x size)',
			isWinner: false,
		},
		'cluster-ellipse-4x-center-close-1': {
			title: 'Elliptical 4× + Close Center 1',
			description:
				'Lenticular cluster with 70 closer center stars (2-5x closer)',
			isWinner: false,
		},
		'cluster-ellipse-4x-center-close-2': {
			title: 'Elliptical 4× + Close Center 2',
			description:
				'Lenticular cluster with 80 closer + concentrated center stars (2.5-6x closer)',
			isWinner: false,
		},
	}
